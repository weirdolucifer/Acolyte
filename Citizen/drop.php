    ...
    <center>
      ...
      <br>
                 <?php
                    ...
                    if(isset($_POST['value']) && isset($_POST['value1']))
                    {

                    
                    $Zone = trim(strip_tags($_POST['value']));
                    $Location = trim(strip_tags($_POST['value1']));

                     mysql_connect("localhost", "root", "") or die(mysql_error()); 
                     mysql_select_db("Law") or die(mysql_error()); 
                     $query= mysql_query("SELECT * FROM `PS` WHERE `Zone` = '".$Zone."' AND `Location` = '".$Location."' ")or die(mysql_error());

                    

                     $arr = mysql_fetch_array($query);
                     $raw_results = mysql_query("SELECT * FROM Department WHERE Station_Code='".$arr['Station_Code']."'") or die(mysql_error());
                     $raw_results1 = mysql_query("SELECT * FROM Streets WHERE Station_Code='".$arr['Station_Code']."'") or die(mysql_error());
                     ?>
                ...
                <center>
                  <form >
                  <a href="profile.php#PS" class="btn btn-outline-success my-2 my-sm-0" type="submit"> Back to previous page</a>
                  </form>
                </center>
                <?php
              }
